...
async function doTambahProduk() {
  ...
  try {
    ...
    // Validasi sinkron: pastikan produk benar-benar tersimpan.
    // (Menghindari kasus redirect sebelum storage update.)
    const pAfter = JSON.parse(localStorage.getItem(STORAGE_PRODUCTS_KEY) || "[]");
    if (!Array.isArray(pAfter) || !pAfter.some(x => Number(x.id) === newId)) {
      throw new Error("Data tidak ditemukan di storage setelah simpan.");
    }
